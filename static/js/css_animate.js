...
class Menu {
  constructor () {
    ...
    this._menuItem = this._menu.querySelector('.menu__items');
  ...
  toggle () {
    if (this._expanded) {
	this._menuTitle.classList.remove('toggle-open');
      this.collapse();
      return;
    }

      this._menuTitle.classList.add('toggle-open');
    this.expand();
  }

  _addEventListeners () {
    this._menuToggleButton.addEventListener('click', this.toggle);
    this._menuItem.addEventListener('click', this.toggle);
  }
